class Client {
    ...
    format_amount = (amount, decimals = 2) => {
    ...
    enable_nav_menu_controllers() {
        $('#sidenav ul li').click((e) => {
            e.preventDefault();
            $('#sidenav ul li').removeClass('border')
    ...
    init_print_format() {
        ...
        if (cur_url.includes('print_formats')) {
            ...
            if (receipt_id) {
                ...
                $.getJSON(`${this.baseUrl}/cash.php?details=${pay_id}`, (response)=>{
                    ...
                    <thead>
                        <tr class="text-bold">
                            ...
                            <th class="border p-2">Subprogram</th>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="border p-2">${other[0].subprogram}</td>
                        ...
                        <td class="border p-2"></td>
                        <td class="border p-2"></td>
                        <th colspan="5" class="border p-2">${cash[0].amount}</th>
                        </tr>
                    </tbody>
                </table>
                `
    ...
    loader = () => {
        return `<div class="p-4 m-4 flex justify-center"><i class=" text-blue-500 fa fa-spinner fa-5x fa-spin"></i></div>`
    }

    // fetching html 
    async get_page() {
        const url = $('#sidenav ul .border').attr('target') || ''
        ...
        this.fetch_data(this.baseUrl + '/customer.php?allcustomers=1').then(resolve => {
            ...
            if (resolve.status) {
                ...
                resolve.data.length > 0 && resolve.data.map((row, i) => {
                    total_rows += 1
                    rows += `
                        <div class="w-full tr grid grid-cols-14 h-[50px] border-b text-[13px] gap-x-4">
                            <span class="row_object hidden" row='${JSON.stringify(row)}'></span>
                                <div class="th flex items-center h-full w-full col-span-2 ml-3">${i + 1}</div>
                                ...
                                <div class="th flex items-center h-full w-full col-span-2">${row.firstname} ${row.middlename} ${row.lastname}</div>
                                <div class="th flex items-center h-full w-full col-span-2">${row.phonenumber}</div>
                                <div class="th flex items-center h-full w-full col-span-4 w-full">
                                    <button class="view-customer m-2">
                                        <i class="fa fa-pencil text-primary mr-2" aria-hidden="true"></i>
                                    </button>
        ...
        this.fetch_data(this.baseUrl + '/reciepts.php?receipts').then(resolve => {
            if (resolve?.data?.length > 0) {
                ...
                $('.print-receipt').click(function () {
                    var newTab = window.open(`/print_formats/receipt.html?rid=${$(this).attr('id')}`, '_blank');
                    newTab.focus();
                })
